...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile = Sprite([self.pos[0] + self.forward[0] * self.radius, self.pos[1] + self.forward[1] * self.radius], \
                           [self.vel[0] + 2 * self.forward[0], self.vel[1] + 2 * self.forward[1]], \
...
def rock_spawner():
    global a_rock
    a_rock = Sprite([random.randint(0, WIDTH), random.randint(0, HEIGHT)], [random.randint(-1,1), random.randint(-1,1)], 0, 0.1, asteroid_image, asteroid_info)

def keydown(key):
    global my_ship

    if key == simplegui.KEY_MAP['left']:
        my_ship.angle_vel -= 0.03 # Magic number fix this
...
a_missile = Sprite([-5,5], [0,0], 0, 0, missile_image, missile_info, missile_sound)
